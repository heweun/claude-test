...
- 충돌 있음 → **원래 브랜치로 복원하고 중단**:
  ...
  사용자에게 보고:
  ```
  ⚠️ 충돌 감지: origin/$BASE_BRANCH와 $SOURCE_BRANCH 사이에 충돌이 있습니다.

  충돌 파일: [git diff --name-only --diff-filter=U 결과]
  ...
  해결 방법:
    ...
    3. 충돌 파일 직접 수정
    4. git add <파일>
    5. git commit -m "fix: $BASE_BRANCH 최신화 충돌 해결"
    6. 다시 /merge-pr 실행
  ```
